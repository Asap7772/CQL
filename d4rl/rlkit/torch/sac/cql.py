...
class CQLTrainer(TorchTrainer):
    ...
    def __init__(
        ...
        self.modify_func_const = modify_func_const
    ...
    def train_from_torch(self, batch):
        ...
        if self.modify_func:
            if self.modify_func_type == 'exp':
                def f(x):
                    return torch.exp(x/self.modify_func_const)
                g = torch.log
            else:
                assert False

            min_qf1_loss = torch.logsumexp(f(cat_q1) / self.temp, dim=1,).mean() * self.temp
            min_qf2_loss = torch.logsumexp(f(cat_q2) / self.temp, dim=1,).mean() * self.temp
                    
            """Subtract the log likelihood of data"""
            min_qf1_loss = g(min_qf1_loss - f(q1_pred).mean()) * self.min_q_weight
            min_qf2_loss = g(min_qf2_loss - f(q2_pred).mean()) * self.min_q_weight
